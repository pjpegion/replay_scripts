...
def get_exec_path(exec_name, verbose=False):
    ...
    # assumes executable lives in $HOME/bin
    homedir=os.getenv('HOME')
    exec_path_def = os.path.join(os.path.join(homedir,'bin'),exec_name)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-d', '--debug', help='run in debug mode', action='store_true', required=False)
    ...
    cmd = '%s -f nc4 -z zip_4 import_binary %s.ctl %s.nc4' % (cdo, finput, fname)
    execute_subprocess(cmd, verbose=verbose)
